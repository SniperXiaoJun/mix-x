...
#if defined(DB_CRYPTO_USE_ANDROID_KEY_STORE)
...
extern jclass g_KeyStoreProcessClass;

int DbCryptoUseAndroidKeyStore(int crypto, unsigned char* data, int len)
{
    JavaVM *jvm;
    JNIEnv *env;
    ...
    int rv = 0;
    int wasAttached = 1;
    jclass jKeyStoreProcessClass;
    jmethodID methodID;
    jfieldID fieldID;
    jbyteArray jValueBefore, jValueAfter;
    unsigned char * pbValueBefore;
    unsigned char * pbValueAfter;
    ...
    if (returnValue == JNI_EDETACHED) {
		FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "JNI_EDETACHED");

        /* thread detached, so attach it */
        wasAttached = 1;
        returnValue = (*jvm)->AttachCurrentThread(jvm, (void **) &env, NULL);

		FILE_LOG_FMT(file_log_name, "%s %d returnValue = %d", __FUNCTION__, __LINE__, returnValue);
    } else if (returnValue == JNI_EVERSION) {
		FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "JNI_EVERSION");
	...
	if (g_KeyStoreProcessClass)
	{
		jKeyStoreProcessClass = g_KeyStoreProcessClass;
	}
	else
	{
		//FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "jKeyStoreProcessClass");

		//jKeyStoreProcessClass = (*env)->FindClass(env, "com/gotrust/vse/KeyStoreProcessClass");

		//FILE_LOG_FMT(file_log_name, "%s %d %d", __FUNCTION__, __LINE__, jKeyStoreProcessClass);

		//FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "");

		//g_KeyStoreProcessClass = (jclass)(*env)->NewGlobalRef(env,jKeyStoreProcessClass);
	}
    ...
    if(crypto)
    {
        ...
        methodID = (*env)->GetStaticMethodID(env, jKeyStoreProcessClass, "encode", "([BI)[B");
		FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "");
    }
    else
    {
		FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "");
        /* call the CK_LOCKMUTEX method of the LockMutex object */
        methodID = (*env)->GetStaticMethodID(env, jKeyStoreProcessClass, "decode", "([BI)[B");
		FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "");
    }

	FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "");
    
    jValueBefore = (*env)->NewByteArray(env,len);
    
    (*env)->SetByteArrayRegion(env,jValueBefore, 0, len, data);
    
    FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "jValueBefore");
    FILE_LOG_FMT(file_log_name, "%s %d %d", __FUNCTION__, __LINE__, (*env)->GetArrayLength(env, jValueBefore));
    
    pbValueBefore = (*env)->GetByteArrayElements(env, jValueBefore, JNI_FALSE);
    
    FILE_LOG_HEX(file_log_name, pbValueBefore, (*env)->GetArrayLength(env, jValueBefore));
    
    jValueAfter = (*env)->CallStaticObjectMethod(env, jKeyStoreProcessClass, methodID, jValueBefore,0);
    
    FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "jValueAfter");
    FILE_LOG_FMT(file_log_name, "%s %d %d", __FUNCTION__, __LINE__, (*env)->GetArrayLength(env, jValueAfter));
    
    pbValueAfter = (*env)->GetByteArrayElements(env, jValueAfter, JNI_FALSE);
    
    memcpy(data, pbValueAfter, len);
    
    FILE_LOG_HEX(file_log_name, pbValueAfter, (*env)->GetArrayLength(env, jValueAfter));
    
    if(pbValueBefore)
    {
        (*env)->ReleaseByteArrayElements(env, jValueBefore, pbValueBefore, JNI_FALSE);
        FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "free jValueBefore");
        (*env)->DeleteLocalRef(env,jValueBefore);
        FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "DeleteLocalRef jValueBefore");
    }
    
    if(pbValueAfter)
    {
        (*env)->ReleaseByteArrayElements(env, jValueAfter, pbValueAfter, JNI_FALSE);
        FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "free jValueAfter");
        (*env)->DeleteLocalRef(env,jValueAfter);
        FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "DeleteLocalRef jValueAfter");
    }
    ...
    //if(jKeyStoreProcessClass)
    //{
    //    (*env)->DeleteLocalRef(env,jKeyStoreProcessClass);
    //    FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "DeleteLocalRef jKeyStoreProcessClass");
    //}
    
    
    FILE_LOG_FMT(file_log_name, "%s %d %s", __FUNCTION__, __LINE__, "crypto end");
    FILE_LOG_FMT(file_log_name, "%s %d %d", __FUNCTION__, __LINE__, crypto);
...
{
    DbCryptoUseAndroidKeyStore(1, data, len);
}

void
CodecDecrypt(Codec* codec, int page, unsigned char* data, int len)
{
    DbCryptoUseAndroidKeyStore(0, data, len);
}


#elif defined(DB_CRYPTO_USE_KEYCHAIN)

void DbCryptoUseKeyChain(int crypto, unsigned char* srcData,unsigned int srcLen,unsigned char * destData, unsigned int destLen);

void
CodecEncrypt(Codec* codec, int page, unsigned char* data, int len, int useWriteKey)
{
    DbCryptoUseKeyChain(1, data, len,data,len);
}

void
CodecDecrypt(Codec* codec, int page, unsigned char* data, int len)
{
     DbCryptoUseKeyChain(0, data, len,data,len);
}
