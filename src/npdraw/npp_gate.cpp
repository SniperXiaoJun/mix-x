...
int16_t	NPP_HandleEvent(NPP instance, void *event)
{
	...
	return rv;
#else
	NPEvent * pNPEnent = (NPEvent *)event;
	switch (pNPEnent->event) {
	case WM_PAINT:
	{
		RECT * drc = (RECT *)pNPEnent->lParam;
		HDC hDC = (HDC)pNPEnent->wParam;

		DrawTextA(hDC, "hello world!", strlen("hello world!"), drc, DT_SINGLELINE | DT_CENTER | DT_VCENTER);

		MessageBoxA(NULL, "WM_PAINT", "info", 0);
	}
	break;
	default:
		break;
